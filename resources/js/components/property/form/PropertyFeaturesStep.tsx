...
export default function PropertyFeaturesStep({ data, setData, featureGroups, attributes, setAttributes, errors, nextStep, prevStep }: any) {
  return (
    <div>
        ...
        <div className="space-y-2">
          {attributes.map((attr: any, idx: number) => (
            <div key={idx} className="flex gap-2 items-center">
              <input type="text" placeholder="Attribute Name" value={attr.key} onChange={e => { const newAttrs = [...attributes]; newAttrs[idx].key = e.target.value; setAttributes(newAttrs); }} className="w-1/2 input input-bordered" />
              <input type="text" placeholder="Value" value={attr.value} onChange={e => { const newAttrs = [...attributes]; newAttrs[idx].value = e.target.value; setAttributes(newAttrs); }} className="w-1/2 input input-bordered" />

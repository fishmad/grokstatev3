...
class listing_pages {
			...
			if ($field_type == 'divider') {
				...
				if ($display_type === 'both' || $display_type === 'value') {
					...
					elseif ($field_type == "url") {
						...
						$display .= $field_value;
					}elseif ($field_type == "date" OR $field_type == "date-time") {
	...
	function renderTemplateArea($templateArea, $listingID)	{
		...
		while (!$recordSet->EOF) {
            ...
            $display .= '
              <div class="panel panel-default">
                <div class="panel-heading">
                  <h3 class="panel-title">' . $field_caption . '</h3>
                </div>
            ';
						$feature_index_list = explode("||", $field_value);
						sort($feature_index_list);
						...
						$display .= '<ul class="nav nav-sidebar">';
							foreach($feature_index_list as $feature_list_item) {
								if ($l < $list_count) {
									$display .= '<li class="list-group-item"><span class="badge"><span class="glyphicon glyphicon-ok" aria-hidden="true"></span></span>';
									$display .= $feature_list_item;
									$display .= $config['feature_list_separator'];
									$display .= '</li>';
									$l++;
            ...
            ';
					...
					elseif ($field_type == "url") {
						$display .= "<strong>$field_caption</strong>: <a href=\"$field_value\" onclick=\"window.open(this.href,'_blank','location=1,resizable=1,status=1,scrollbars=1,toolbar=1,menubar=1');return false\">$field_value</a>" . '<br />';
					}elseif ($field_type == "email") {
						...
						$display .= "<strong>$field_caption</strong>: ";
            $display .= "$field_value<br />";
  ...
  function get_notes($listing_id)	{
		global $conn, $config;
		require_once($config['basepath'] . '/include/misc.inc.php');
		$misc = new misc();
		$listing_id = $misc->make_db_extra_safe($listing_id);
		$sql = "SELECT listingsdb_notes FROM " . $config['table_prefix'] . "listingsdb WHERE listingsdb_id = $listing_id";
		$recordSet = $conn->Execute($sql);
		if ($recordSet === false) {
			$misc->log_error($sql);
		}
	...
	function getListingAgentLink($listingID)	{
		...
		if ($config['url_style'] == '1') {
			$display .= "index.php?action=view_user&amp;user=$listing_user_ID";
		}else {
			...
			$display .= 'agent-' . urlencode($url_title) . '-' . $listing_user_ID . '.html';
		}
		return $display;
	}
	...
	function getAgentListingsLink($listing_id)	{
		...
		$display .= '<a href="index.php?action=searchresults&amp;user_ID='.$listing_user_ID.'">'.$lang['user_listings_link_text'].'</a>';
		return $display;
	}
...
//$new_row_data[] = $featured_template_section;
				...
				if ($config['url_style'] == '1') {
					...
					$featured_url = 'property' . misc::url_seo_clean($listing_title) . $current_ID . '.html';
				}
...
// gotta grab the thumbnail image size
	...
	// gotta grab the thumbnail image size
						...
						if ($config['show_no_photo'] == 1) {
							$imagedata = GetImageSize($config["basepath"] . '/images/photo_not_available_sm.jpg');
							$imagewidth = $imagedata[0];
							$imageheight = $imagedata[1];
							$shrinkage = $config['thumbnail_width'] / $imagewidth;
							$featured_thumb_width = $imagewidth * $shrinkage;
	...
	function create_email_friend_link($url_only = 'no')	{
		global $lang,$config;
		if ($url_only == 'no') {
			$display = '<a href="index.php?action=contact_friend&amp;popup=yes&amp;listing_id=' . $_GET['listingID'] . '" onclick="window.open(this.href,\'_blank\',\'location=0,status=0,scrollbars=1,toolbar=0,menubar=0,width=500,height=520\');return false">' . $lang['email_listing_link'] . '</a>';
			} else {
				$display = 'index.php?action=contact_friend&amp;popup=yes&amp;listing_id=' . $_GET['listingID'] . '';
			}
		return $display;
	}
	...
	function create_printer_friendly_link($url_only = 'no')	{
		...
		if (isset($_GET['listingID'])) {
			if ($url_only == 'no') {
				...
				$display = 'index.php?action=listingview&amp;listingID=' . $_GET['listingID'] . '&amp;printer_friendly=yes';
			}
		}else {
			// Save GET
			$guidestring = '';
			...
			if ($url_only == 'no') {
				$display = '<a href="index.php?' . $guidestring . '&amp;printer_friendly=yes">' . $lang['printer_version_link'] . '</a>';
			} else {
				$display = 'index.php?' . $guidestring . '&amp;printer_friendly=yes';
			}
		}
		return $display;
	}
	...
	function create_calc_link($url_only = 'no')	{
		global $lang,$config;
		if ($url_only == 'no') {
		$display = '<a href="index.php?action=calculator&amp;popup=yes&amp;price=' . listing_pages::renderSingleListingItem($_GET['listingID'], $config['price_field'], 'rawvalue') . '" onclick="window.open(this.href,\'_blank\',\'location=0,status=0,scrollbars=1,toolbar=0,menubar=0,width=500,height=520\');return false">' . $lang['mortgage_calculator_link'] . '</a>';
		} else {
		$display = 'index.php?action=calculator&amp;popup=yes&amp;price=' . listing_pages::renderSingleListingItem($_GET['listingID'], $config['price_field'], 'rawvalue') . '';
		}
		return $display;
	}
	...
	function create_add_favorite_link($url_only = 'no')	{
		...
		if ($url_only == 'no') {
		$display = '<a href="index.php?action=addtofavorites&amp;listingID=' . $_GET['listingID'] . '">' . $lang['add_favorites_link'] . '</a>';
		} else {
		$display = 'index.php?action=addtofavorites&amp;listingID=' . $_GET['listingID'] . '';
		}
		return $display;
	}
